...
var generateBookList=function(data,param){
	var sHtml = '';
	...
	$.each(data.data.items, function(i, n){
		...
		sHtml += '                	<h5 class="add-title"><a href="'+n.out_link+'" target="_blank">'+webApp.myhtmlencode(n.book_name)+'</a></h5>';
...
var callBackGetBookByPtag = function(data, param){
	var sHtml = generateBookList(data,param);
...
var callBackGetBookByTag = function(data, param){
...
var callBackDefaultTags = function(data){
	...
	$.each(data.data.items, function(i, n){
		...
		if(i == 0) {
			getBookByPtag(n.id, 1, true);
		} else {
			getBookByPtag(n.id, 1, false);
		}
...
//按大类搜索图书
function getBookByPtag(ptag, page, is_show) {
	var keyword = $.trim($("#form_search input[type='text']").val());
	var urlStr = 'book/all_books?book_name='+encodeURIComponent(keyword)+'&ptag='+ptag+'&page='+page+'&limit='+_limit;
	var dataJson = {};
	webApp.postRequest(urlStr, dataJson, callBackGetBookByPtag, {"ptag":ptag, "is_show":is_show, "page":page});
}

//获取默认大分类
function getDefaultTags() {
	var urlStr = 'goods/default_tags?category_id=' + webApp.book_category_id;
	var dataJson = {};
...
function getBookByTags(page) {
	...
	// if(obj.find("input[type='checkbox']:checked").length) {
		...
		var urlStr = 'book/tags_books?tag_ids='+tag_ids+'&page='+page+'&limit=' + _limit;
		var dataJson = {};
        webApp.postRequest(urlStr, dataJson, callBackGetBookByTag, {"page":page});
